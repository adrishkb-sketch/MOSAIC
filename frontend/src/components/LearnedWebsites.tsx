      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {websites.map((site) => (
            <div
              key={site.id}
              className="glass-panel hover:border-slate-400/50 dark:border-white/20 transition-all rounded-3xl p-6 space-y-5"
            >
              {/* Site Header details */}
              <div className="flex items-start justify-between border-b border-slate-300/50 dark:border-white/10 pb-4">
                <div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {Object.entries(parseJSON(site.workflows, {})).map(([name, desc]: [string, any], idx) => (
                    <div key={idx} className="bg-white/40 dark:bg-black/20 backdrop-blur-md border border-slate-300/50 dark:border-white/10 p-2.5 rounded-xl text-xs">
                      ...
                      <p className="text-slate-700 dark:text-slate-300 font-medium leading-relaxed">{desc}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1 bg-white/40 dark:bg-black/20 backdrop-blur-md border border-slate-300/50 dark:border-white/10 p-3 rounded-xl">
                  {parseJSON(site.commands, []).map((cmd: string, idx: number) => (
                    <code key={idx} className="block text-slate-600 dark:text-slate-400 font-mono text-[10px] leading-relaxed select-all">
                      $ {cmd}
                    </code>
                  ))}
                </div>
              ...
              <div className="text-[10px] text-slate-550 border-t border-slate-300/50 dark:border-white/10 pt-4 flex justify-between font-medium">
